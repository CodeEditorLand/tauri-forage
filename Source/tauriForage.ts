...
import { handler, LoggerType, MaybeLoggerType, ReturnerType } from "./handler";
...
const forage = {
		...
		return async function (curry?: MaybeFunction) {
			...
			return handler.returner(
				storage
					...
					.then(async (v: any) => {
						return !before
							? handler.maybeCurry(curry || null)(v)
							: v;
					})
					.catch((err: any) => {
			...
			return handler.returner(
				storage
					...
					.then((v: any) => {
						// console.log(v)
						return !before
							? handler.maybeCurry(curry || null)(v[value])
							: v[value];
					})
					.catch((err: any) => {
						/* istanbul ignore next */
							...
							case "custom":
								try {
									val = await storage.setItem(
										key,
										await handler.maybeCurry(curry)(v),
									);
							...
							case "withKey":
								try {
									return storage.setItem(
										key,
										mergeWithKey(concatValues, v, value),
									);
								} catch (err) {
									/* istanbul ignore next */
									return handler.logger(err)(logger);
	...
	clear({
		logger = "none",
		store,
	}: { logger?: LoggerType; store?: string } = {}) {
		return async function (curry?: MaybeFunction) {
			const storage = await _defineStore({ store: store });
	...
	length({
		...
		returner,
		store,
	}: { logger?: LoggerType; returner?: ReturnerType; store?: string } = {}) {
		return async function (curry?: MaybeFunction) {
			const storage = await _defineStore({ store: store });
  ...
  */
	keys({
		...
		returner,
		store,
	}: { logger?: LoggerType; returner?: ReturnerType; store?: any } = {}) {
		return async function (curry: MaybeFunction) {
			const storage = await _defineStore({ store: store });
	...
	hasKey({ key, logger, returner, store }: Item = {}) {
		// must be a key
		return async function (curry?: MaybeFunction) {
			const storage = await _defineStore({ store: store });
			...
			return handler.returner(
				await storage
					.keys()
					.then((k: string[]) => {
						return handler.maybeCurry(curry || null)(
							k.includes(key),
						);
					})
					.catch((err: any) => {
  ...
  */
	hasKeyValue({
		...
		// boolean version of getKeyValue
		return async function (curry?: MaybeFunction) {
			return handler.returner(
				await localForage
					.getItem(key)
